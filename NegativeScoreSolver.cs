...
namespace DotNet
{
    public class NegativeScoreSolver
    {
        ...
        public NegativeScoreSolver(List<Package> packages, Vehicle vehicle)
        {
            /*
        ...
        {
            Console.WriteLine(_packages.Sum(item => item.Height * item.Length * item.Width) / (double)(_truckX * _truckY * _truckZ));
            var groups = _packages.GroupBy(item => item.OrderClass / 2).OrderBy(item => item.Key).Select(item => item.AsEnumerable()).ToList();
            int grpidx = 0;
            foreach (var group in groups)
            {
                ...
                var packages = group.OrderBy(item => item.OrderClass).ThenBy(item => item.WeightClass == 2 ? item.WeightClass : (item.WeightClass == 1 ? 0 : 1)).ThenByDescending(item => Max(item.Width, item.Height, item.Length));
                ...
                {
                    for (int j = 0; j < _truckX; j++)
                    {
                        for (int k = 0; k < _truckZ; k++)
                        {
                            _used[i, k, j] = true;
                        }
                    }
