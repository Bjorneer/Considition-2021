...
namespace DotNet.Generators
{
    public class EasterGenerator : Generator
    {
        ...
        private int GetRandomPackageSize()
        {
            ...
            return (int)Math.Max(Math.Min(Math.Round(Math.Sqrt(-2.0 * Math.Log(u)) * Math.Cos(2.0 * Math.PI * v) * 3 + 18), 70), 5);
        }
        ...
        protected override void ReGenerate()
        {
            ...
            int height = 100;
            int widht = 120;
            double[] weightProbabilities = new double[] { 0.6, 0.7, 1 };
            double[] orderProbabilities = new double[] { 0.2, 0.4, 0.6, 0.8, 1 };
            double aprxPackVolumeOfTotal = 0.3;
